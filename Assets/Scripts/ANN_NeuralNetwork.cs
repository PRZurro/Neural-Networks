...
public class ANN_NeuralNetwork{

    public ANN_Layer m_input, m_hide, m_output;
        

    public ANN_NeuralNetwork(int inputNumberOfNeurons, int hideNumberOfNeurons, int outputNumberOfNeurons) {
        ...
        if (inputTraining.GetLength(1) == m_input.m_numberOfNeurons && 
            ...
            while ((error > errorTolerance) && (actualEpoch < maxEpoch)) {
                ...
                for (int i = 0; i < inputTraining.GetLength(0); i++) {
                    InputNetwork(BidimensionalToUnidimensional(inputTraining, i));
                    SetDesiredOutput(BidimensionalToUnidimensional(desire, i));
                    FeedForward();
    ...
    float[] BidimensionalToUnidimensional( float[,] matrix, int position) {
        
        float[] unidimensional =  new float[0];
        if (matrix.GetLength(0) > position) {
            unidimensional = new float[matrix.GetLength(1)];
            for (int i = 0; i < unidimensional.Length; i++) {            
                unidimensional[i] = matrix[position, i];
            }
        } 
    ...
    public int AskAction(float[] input) {
        InputNetwork(input);
        FeedForward();
